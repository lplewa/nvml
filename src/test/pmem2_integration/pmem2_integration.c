...
test_map_from_existing_map(const struct test_case *tc, int argc, char *argv[])
{
	...
	UT_ASSERTeq(pmem2_map_from_existing(&map2, src,
			pmem2_map_get_address(map1), pmem2_map_get_size(map1),
			pmem2_map_get_store_granularity(map1)), PMEM2_E_MAP_EXISTS);
...
#ifdef _WIN32
/*
 * map_file -- map file without using pmem2 api
 */
static void *
map_file(int fd, size_t size)  {
	void *addr = NULL;
	HANDLE handle = (HANDLE)_get_osfhandle(fd);
	UT_ASSERTne(mh, INVALID_HANDLE_VALUE);
